      ...
      <div className="bg-[#1a1f2e] p-8 rounded-[32px] border border-slate-800 shadow-2xl flex flex-col xl:flex-row justify-between items-center gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center xl:text-left w-full xl:w-auto">
          ...
          <p className="text-slate-500 font-bold text-[10px] tracking-[0.3em] uppercase mt-1 flex items-center justify-center xl:justify-start gap-2">
            <Wallet size={14} className="text-emerald-500" /> Fundos de Campanhas e Acampamentos
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 w-full xl:w-auto relative z-10 justify-end items-center">
          ...
          <div className="flex gap-2 flex-wrap sm:flex-nowrap w-full lg:w-auto justify-center lg:justify-end">
            ...
            <button 
              ...
              className="bg-slate-800/50 hover:bg-slate-800 border border-slate-700 text-slate-300 px-4 py-3 rounded-2xl flex items-center justify-center gap-2 font-black uppercase text-[10px] tracking-widest transition-all flex-1 sm:flex-none whitespace-nowrap"
      ...
      {!isLoading && filteredYouths.length > 0 && (
        <div className="flex justify-end px-2 mb-4 relative z-10">
          <button 
            ...
            className="flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-white transition-colors uppercase tracking-widest bg-[#0f172a]/50 py-2 px-4 rounded-xl border border-slate-800/50 hover:border-slate-700"
          >
            <span className={`w-4 h-4 rounded border flex items-center justify-center ${selectedYouths.length === filteredYouths.length ? 'bg-blue-500 border-blue-500 text-white' : 'border-slate-600 text-transparent'}`}>
              <CheckSquare size={12} />
            </span>
            ...
            {filteredYouths.map((youth) => {
              ...
              return (
                <motion.div 
                  ...
                  className={`relative bg-[#1a1f2e] border p-6 rounded-[28px] transition-all group shadow-xl flex flex-col justify-between cursor-pointer
                    ${isSelected ? 'border-blue-500 bg-blue-500/5' : 'border-slate-800 hover:border-emerald-500/30'}
                  `}
                >
                  <div className="absolute top-4 right-4 z-10">
                    <div className={`w-5 h-5 rounded flex items-center justify-center border transition-all ${isSelected ? 'bg-blue-500 border-blue-500 text-white' : 'bg-[#0f172a] border-slate-700 text-transparent group-hover:border-slate-500'}`}>
                      <CheckSquare size={14} />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className="pr-6">
                      ...
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">
                        Ramo <span className={`${isSelected ? 'text-blue-500' : 'text-emerald-500'} italic`}>{youth.branch}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-800/50 flex justify-between items-end">
                    <div>
                      ...
                      <p className={`text-3xl font-black italic tracking-tighter transition-colors ${balance > 0 ? 'text-emerald-400' : balance < 0 ? 'text-red-500' : 'text-slate-400'}`}>
                        R$ {balance.toFixed(2)}
                      </p>
                    </div>
                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-colors ${balance > 0 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : balance < 0 ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-[#0f172a] border-slate-800 text-slate-600'}`}>
                      <Wallet size={18} />
                    </div>
                  </div>
                </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-[#1a1f2e] border border-slate-800 rounded-[32px] p-8 max-w-md w-full shadow-2xl relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-8 relative z-10">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-amber-500 uppercase tracking-widest mt-1">Descontos do Caixa</p>
                </div>
              ...
              <form onSubmit={handleExportActivity} className="space-y-5 relative z-10">
                <div>
                  ...
                  <input required type="text" value={activityName} onChange={e => setActivityName(e.target.value)} placeholder="Ex: Acampamento Regional" className="w-full bg-[#0f172a] border border-slate-800 rounded-2xl p-4 text-slate-200 focus:border-amber-500/50 outline-none text-sm font-medium transition-all" />
                </div>
                <div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-[#1a1f2e] border border-slate-800 rounded-[32px] p-8 max-w-md w-full shadow-2xl relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-8 relative z-10">
                <div>
                  <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">Lançamento</h2>
                  <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mt-1">Conta Individual</p>
                </div>
              ...
              <form onSubmit={handleTransaction} className="space-y-5 relative z-10">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button type="button" onClick={() => setType('credit')} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${type === 'credit' ? 'border-emerald-500 bg-emerald-500/10 text-emerald-500' : 'border-slate-800 bg-[#0f172a] text-slate-500 hover:border-slate-700'}`}>
                    <TrendingUp size={24} /> <span className="text-[10px] font-black uppercase tracking-widest">Entrada (+)</span>
                  </button>
                  <button type="button" onClick={() => setType('debit')} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${type === 'debit' ? 'border-amber-500 bg-amber-500/10 text-amber-500' : 'border-slate-800 bg-[#0f172a] text-slate-500 hover:border-slate-700'}`}>
                ...
                <div>
                  ...
                  <input required type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Motivo..." className="w-full bg-[#0f172a] border border-slate-800 rounded-2xl p-4 text-slate-200 focus:border-emerald-500/50 outline-none text-sm font-medium transition-all" />
                </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-[#1a1f2e] border border-slate-800 rounded-[32px] p-8 max-w-md w-full shadow-2xl relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-6 relative z-10">
                <div>
                  <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">Lote: {selectedYouths.length} Jovens</h2>
                  <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mt-1">Lançamento em Massa</p>
                </div>
                <button onClick={() => setIsBatchModalOpen(false)} className="text-slate-500 hover:text-red-500 transition-colors p-2 bg-[#0f172a] border border-slate-800 rounded-xl"><X size={20} /></button>
              </div>

              <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 mb-6 relative z-10">
                <p className="text-xs font-medium text-blue-400 text-center">
              ...
              <form onSubmit={handleBatchSubmit} className="space-y-5 relative z-10">
                <div className="grid grid-cols-2 gap-4">
                  <button type="button" onClick={() => setBatchData({...batchData, type: 'credit'})} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${batchData.type === 'credit' ? 'border-emerald-500 bg-emerald-500/10 text-emerald-500' : 'border-slate-800 bg-[#0f172a] text-slate-500 hover:border-slate-700'}`}>
                    <TrendingUp size={24} /> <span className="text-[10px] font-black uppercase tracking-widest">Entrada (+)</span>
                  </button>
                  <button type="button" onClick={() => setBatchData({...batchData, type: 'debit'})} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${batchData.type === 'debit' ? 'border-amber-500 bg-amber-500/10 text-amber-500' : 'border-slate-800 bg-[#0f172a] text-slate-500 hover:border-slate-700'}`}>
                    <TrendingDown size={24} /> <span className="text-[10px] font-black uppercase tracking-widest">Saída (-)</span>
                  </button>
                </div>
                ...
                <div>
                  ...
                  <input required type="number" step="0.01" min="0.01" value={batchData.amount} onChange={e => setBatchData({...batchData, amount: e.target.value})} placeholder="0.00" className="w-full bg-[#0f172a] border border-slate-800 rounded-2xl p-4 text-white text-xl font-black italic focus:border-blue-500/50 outline-none transition-all" />
                </div>
                
                <div>
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Descrição (Ex: Acampamento)</label>
                  <input required type="text" value={batchData.description} onChange={e => setBatchData({...batchData, description: e.target.value})} placeholder="Acampamento de Grupo..." className="w-full bg-[#0f172a] border border-slate-800 rounded-2xl p-4 text-slate-200 focus:border-blue-500/50 outline-none text-sm font-medium transition-all" />
                </div>

                <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black uppercase tracking-widest text-xs p-4 rounded-2xl disabled:opacity-50 mt-4 shadow-lg flex items-center justify-center gap-2 transition-all">
