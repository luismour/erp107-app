...
export default function MensalidadesPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10 px-4 min-h-screen transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-[#1a1f2e] p-8 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl flex flex-col xl:flex-row justify-between items-center gap-6 relative overflow-hidden transition-colors duration-300">
        ...
        <div className="relative z-10 text-center xl:text-left w-full xl:w-auto">
          <h1 className="text-4xl font-black text-slate-900 dark:text-white italic tracking-tighter uppercase transition-colors">Mensalidades</h1>
          <p className="text-slate-500 font-bold text-[10px] tracking-[0.3em] uppercase mt-1 flex items-center justify-center xl:justify-start gap-2">
            <FileText size={14} className="text-emerald-600 dark:text-emerald-500" /> Gestão Financeira de Sócios
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-800 p-5 rounded-[24px] flex items-center gap-4 shadow-sm dark:shadow-lg transition-colors"><div className="w-12 h-12 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 flex items-center justify-center"><CheckCircle size={20} /></div><div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Recebido</p><p className="text-2xl font-black text-slate-900 dark:text-white transition-colors">R$ {totalRecebido.toFixed(2)}</p></div></div>
        <div className="bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-800 p-5 rounded-[24px] flex items-center gap-4 shadow-sm dark:shadow-lg transition-colors"><div className="w-12 h-12 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-500 flex items-center justify-center"><Clock size={20} /></div><div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">A Receber</p><p className="text-2xl font-black text-slate-900 dark:text-white transition-colors">R$ {totalPendente.toFixed(2)}</p></div></div>
        <div className="bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-800 p-5 rounded-[24px] flex items-center gap-4 shadow-sm dark:shadow-lg transition-colors"><div className="w-12 h-12 rounded-xl bg-red-500/10 text-red-600 dark:text-red-500 flex items-center justify-center"><AlertCircle size={20} /></div><div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Em Atraso</p><p className="text-2xl font-black text-slate-900 dark:text-white transition-colors">R$ {totalAtrasado.toFixed(2)}</p></div></div>
      </div>

      <div className="flex flex-col md:flex-row gap-4 bg-white dark:bg-[#0f172a] p-3 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-inner items-center transition-colors">
        <div className="flex items-center gap-2 pl-2 pr-4 border-r border-slate-200 dark:border-slate-800 text-slate-500 transition-colors"><Filter size={16} /> <span className="text-[10px] font-black uppercase tracking-widest">Filtros</span></div>
        <div className="flex flex-wrap gap-3 flex-1 w-full">
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-slate-50 dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 outline-none focus:border-emerald-500/50 flex-1 md:flex-none transition-colors">
          ...
          <select value={monthFilter} onChange={(e) => setMonthFilter(e.target.value)} className="bg-slate-50 dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 outline-none focus:border-emerald-500/50 flex-1 md:flex-none transition-colors">
            <option value="ALL">Mês: Todos</option>
            {months.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
          </select>
          <select value={yearFilter} onChange={(e) => setYearFilter(e.target.value)} className="bg-slate-50 dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 outline-none focus:border-emerald-500/50 flex-1 md:flex-none transition-colors">
      ...
      <div className="bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-800 rounded-[32px] overflow-hidden shadow-xl dark:shadow-2xl transition-colors">
        {isLoading ? (
          ...
          <div className="text-center py-20 border-dashed border-2 border-slate-200 dark:border-slate-800/50 m-8 rounded-[24px]">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/50 max-h-[600px] overflow-y-auto hide-scrollbar transition-colors">
            <AnimatePresence>
              {filteredFees.map((fee) => {
                ...
                const statusColor = isPaid ? 'text-emerald-600 dark:text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/20' 
                                  : isOverdue ? 'text-red-600 dark:text-red-500 bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/20' 
                                  : 'text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/20'
                const statusLabel = isPaid ? 'PAGO' : isOverdue ? 'ATRASADO' : 'PENDENTE'

                return (
                  <motion.div key={fee.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-slate-50 dark:hover:bg-[#0f172a]/50 transition-colors">
                    <div className="flex items-center gap-4 w-full md:w-auto">
                      ...
                      <div>
                        <h3 className="font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight transition-colors">{fee.youth?.name || "Jovem Removido"}</h3>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-slate-800/50 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700 transition-colors">Ref: {fee.month.toString().padStart(2, '0')}/{fee.year}</span>
                    ...
                    <div className="flex items-center justify-between w-full md:w-auto gap-4 pl-16 md:pl-0">
                      <div className="text-left md:text-right pr-2">
                        ...
                        <p className="text-xl font-black italic tracking-tighter text-slate-800 dark:text-slate-200 mt-1 transition-colors">R$ {Number(fee.amount).toFixed(2)}</p>
                      </div>
                      
                      <div className="flex flex-col sm:flex-row gap-2">
                        {!isPaid && (
                          <button onClick={() => setCarneVirtualFee(fee)} className="bg-blue-500/10 hover:bg-blue-500/20 text-blue-600 dark:text-blue-500 border border-blue-500/30 px-4 py-2 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap">
                            <QrCode size={16} /> Enviar
                          </button>
                        )}
                        {!isPaid && (
                          <button onClick={() => handleConfirmPayment(fee.id)} disabled={isProcessing === fee.id} className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-500 border border-emerald-500/30 px-4 py-2 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50 whitespace-nowrap">
      ...
      <AnimatePresence>
        {isCarneModalOpen && (
          <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-slate-800 rounded-[32px] p-8 max-w-md w-full shadow-2xl relative overflow-hidden transition-colors">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 to-transparent" />
              <div className="flex justify-between items-center mb-8 relative z-10">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 dark:text-white italic tracking-tighter uppercase transition-colors">Gerar Excel</h2>
                  <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-widest mt-1">Planilha Anual p/ Impressão</p>
                </div>
                <button onClick={() => setIsCarneModalOpen(false)} className="text-slate-500 hover:text-red-500 transition-colors p-2 bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-xl"><X size={20} /></button>
              </div>
              ...
              <form onSubmit={handleGenerateCarneExcel} className="space-y-5 relative z-10">
                ...
                <div>
                  ...
                  <select required value={carneYear} onChange={e => setCarneYear(e.target.value)} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl p-4 text-slate-900 dark:text-slate-200 focus:border-emerald-500/50 outline-none uppercase text-xs font-bold transition-all">
